...
void Component::invokeEx( Project& project, FCO& currentFCO, const std::set<FCO>& setSelectedFCOs, long lParam )
{
	...
	int count = Component::ProcessObject(ofs, rootFolder, 0);
	ofs << "Total number of items in structure: " << count << std::endl;
    ofs.close();
    // ======================
...
*/
int Component::ProcessObject(std::ostream &ostr, const Object object, int level)
{
	++level;
	int count = 0;
	std::string indent;
	...
	if (BON::Model(object)) {
		...
		{		
			count += Component::ProcessObject(ostr, *child, level); 
		}
		return count;
